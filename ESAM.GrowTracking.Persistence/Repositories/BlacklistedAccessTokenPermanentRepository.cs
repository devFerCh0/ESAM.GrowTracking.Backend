...
{
    public class BlacklistedAccessTokenPermanentRepository(ILogger<BlacklistedAccessTokenPermanentRepository> logger, AppDbContext context) : Repository<BlacklistedAccessTokenPermanent, int>(logger, context), 
        IBlacklistedAccessTokenPermanentRepository
    {
        public async Task<bool> ExistsAsync(string jti, bool asTracking = false, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var exists = await query.AnyAsync(batp => batp.Jti == jti, cancellationToken);
        ...
        public async Task<int> PurgeExpiredBlacklistedAccessTokensPermanentAsync(int batchSize, DateTime utcNow, bool asTracking = false, CancellationToken cancellationToken = default)
        {
            if (batchSize <= 0)
                batchSize = 1000;
            var totalDeleted = 0;
            ...
            _logger.LogInformation("PurgeExpiredBlacklistedAccessTokensPermanentAsync deleted {TotalDeleted} rows.", totalDeleted);
            return totalDeleted;
        }
    }
}